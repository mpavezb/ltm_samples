...
namespace ltm_samples
{
    ...
    public:
        ...
        void initialize(const std::string& param_ns) {
            ROS_DEBUG_STREAM("LTM Objects Entity plugin initialized with ns: " << param_ns);
        }

        void register_episode(uint32_t uid) {
            ROS_DEBUG_STREAM("LTM Objects Entity plugin: registering episode " << uid);
        }

        void unregister_episode(uint32_t uid) {
            ROS_DEBUG_STREAM("LTM Objects Entity plugin: unregistering episode " << uid);
        }

        void collect(uint32_t uid, ltm::What& msg) {
            ROS_DEBUG_STREAM("LTM Objects Entity plugin: collecting episode " << uid);
        }
    };
